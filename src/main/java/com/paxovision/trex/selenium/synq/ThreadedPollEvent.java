 ...
 * {@link Condition#isMet()}) is gated through the work queue of a single thread
 ...
 * fluent {@link Event} API, each poll event will evaluate their conditions using a
...
public class ThreadedPollEvent<T> implements PollEvent<T> {
     ...
     * {@link TimeKeeper}, and the current thread's associated single thread
